"""
Tier2 Dataset Loader - Parametric Queries

Loads the tier2_parametric_queries dataset which contains queries
with parameters that need to be extracted, plus additional tool calls.
"""
...
@register_dataset("tier2")
class Tier2Dataset(BaseDataset):
    """
    Dataset loader for tier2_parametric_queries.
    
    This dataset contains parametric tool calling queries where
    parameters need to be extracted. May include additional_tool_calls.
    """
    ...
    def load(self) -> None:
        """Load the tier2 dataset from disk."""
        if self._loaded:
            logger.info("Dataset already loaded")
            return
            
        logger.info(f"Loading Tier2 dataset from {self.data_dir}")
    ...
    def _load_query(self, query_dir: Path) -> Optional[QuerySample]:
        ...
        # Parse additional tool calls if present
        additional_tools = []
        ...
        return QuerySample(
            ...
            tier=metadata.get('tier', 'tier2_parametric'),
