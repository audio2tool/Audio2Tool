"""
Tier3 Dataset Loader - Multi-Intent Queries

Loads the tier3_multi_intent_queries dataset which contains queries
with multiple tool calls in sequence.
"""
...
@register_dataset("tier3")
class Tier3Dataset(BaseDataset):
    """
    Dataset loader for tier3_multi_intent_queries.
    
    This dataset contains multi-intent queries where multiple tools
    need to be called in sequence from a single utterance.
    """
    ...
    def load(self) -> None:
        ...
        logger.info(f"Loading Tier3 dataset from {self.data_dir}")
    ...
    def _load_query(self, query_dir: Path) -> Optional[QuerySample]:
        ...
        # Parse additional tool calls (important for multi-intent)
        additional_tools = []
        additional_str = metadata.get('additional_tool_calls', '')
        if additional_str and additional_str != 'nan':
            try:
                # Handle both list format and space-separated format
                if additional_str.startswith('['):
                    additional_tools = ast.literal_eval(additional_str.replace(' ', ', ').replace("'", '"'))
                else:
                    additional_tools = additional_str.split()
            except (ValueError, SyntaxError):
                pass
        ...
        return QuerySample(
            ...
            tier=metadata.get('tier', 'tier3_multi_intent'),
            additional_tool_calls=additional_tools,  # Store in dedicated field
            metadata={
                'tool_id': metadata.get('tool_id'),
                'source_endpoint': metadata.get('source_endpoint'),
                'num_intents': 1 + len(additional_tools),
            }
        )
