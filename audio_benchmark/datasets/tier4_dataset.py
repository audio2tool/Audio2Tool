"""
Tier4 Dataset Loader - Implicit Queries

Loads the tier4_implicit_queries dataset which contains queries
where the tool call must be inferred from context.
"""
...
@register_dataset("tier4")
class Tier4Dataset(BaseDataset):
    """
    Dataset loader for tier4_implicit_queries.
    
    This dataset contains implicit queries where the user's intent
    must be inferred from context rather than explicit commands.
    """
    ...
    def load(self) -> None:
        ...
        logger.info(f"Loading Tier4 dataset from {self.data_dir}")
    ...
    def _load_query(self, query_dir: Path) -> Optional[QuerySample]:
        ...
        # Parse additional tool calls if present (usually empty for tier4)
        additional_tools = []
        ...
        return QuerySample(
            ...
            tier=metadata.get('tier', 'tier4_implicit'),
