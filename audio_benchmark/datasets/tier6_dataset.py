"""
Tier6 Dataset Loader - Correction Queries

Loads the tier6_correction_queries dataset which contains queries
where the user corrects a previous tool call.
"""
...
@register_dataset("tier6")
class Tier6Dataset(BaseDataset):
    """
    Dataset loader for tier6_correction_queries.
    
    This dataset contains correction scenarios where the user
    corrects a previous tool call (e.g., changing parameters).
    Includes original_tool_call and correction_type fields.
    """
    ...
    def __init__(
        ...
        filter_correction_type: Optional[str] = None,
        ...
        **kwargs
        ...
        self.filter_correction_type = filter_correction_type
    ...
    def load(self) -> None:
        ...
        logger.info(f"Loading Tier6 dataset from {self.data_dir}")
    ...
    def _load_query(self, query_dir: Path) -> Optional[QuerySample]:
        ...
        # Filter by correction type if specified
        correction_type = metadata.get('correction_type', '')
        if self.filter_correction_type and correction_type != self.filter_correction_type:
            return None
        ...
        # Get correction-specific fields
        original_tool_call = metadata.get('original_tool_call', '')
        
        # Parse additional tool calls if present (usually empty for tier6)
        additional_tools = []
        ...
        return QuerySample(
            ...
            tier=metadata.get('tier', 'tier6_correction'),
    ...
    def get_correction_types(self) -> List[str]:
        """Get unique correction types in the dataset."""
        return list(set(
            s.metadata.get('correction_type', '') 
            for s in self.samples 
            if s.metadata.get('correction_type')
        ))
