"""
Audio Flamingo 3 vLLM Model Adapter

This adapter connects to one or more running Audio Flamingo 3 vLLM servers
and distributes requests across them using round-robin load balancing.

Audio Flamingo 3 (AF3) is NVIDIA's open-source Large Audio-Language Model
based on Qwen2.5-7B backbone with AF-Whisper unified audio encoder.
It supports audio understanding, ASR, reasoning, and conversation.

The model uses the standard OpenAI-compatible chat API with multimodal
inputs (audio sent as base64 data URLs via the audio_url content type).

Single server usage (1 GPU per server, AF3 is ~7B params):
    # Start server:
    vllm serve nvidia/audio-flamingo-3-hf --port 8801 --dtype bfloat16 \
        --trust-remote-code --max-model-len 4096 --enforce-eager

    # Run benchmark:
    python run_benchmark.py --model audio-flamingo3-vllm ...

Multi-server usage (8 GPUs: 8 servers x 1 GPU each):
    # Server 1 on GPU 0:
    CUDA_VISIBLE_DEVICES=0 vllm serve nvidia/audio-flamingo-3-hf --port 8801
    # Server 2 on GPU 1:
    CUDA_VISIBLE_DEVICES=1 vllm serve nvidia/audio-flamingo-3-hf --port 8802
    # ... etc up to port 8808
...
@register_model("audio-flamingo3-vllm")
class AudioFlamingo3VllmModel(BaseAudioModel):
    """
    Audio Flamingo 3 model adapter that connects to running vLLM server(s).

    Supports multiple servers for parallel inference across GPUs.
    AF3 is ~7B params and fits on a single GPU, so each server needs 1 GPU.
    Auto-discovers servers on ports 8801-8808 by default.
    """

    DEFAULT_BASE_PORT = 8801
    DEFAULT_NUM_PORTS = 8  # Support up to 8 servers (1 GPU each)
    DEFAULT_MODEL_NAME = "nvidia/audio-flamingo-3-hf"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Args:
            api_url: Single server URL (e.g. http://localhost:8801/v1/chat/completions)
            api_urls: Explicit list of server URLs
            base_port: Base port for auto-discovery (default: 8801)
            num_ports: Number of ports to probe starting from base_port (default: 8)
            model_name: Served model name (default: nvidia/audio-flamingo-3-hf)
        """
    ...
    def load_model(self) -> None:
        ...
        if not self._live_urls:
            raise RuntimeError(
                f"No Audio Flamingo 3 vLLM servers reachable. "
                f"Tried: {self._candidate_urls}"
            )

        self._is_loaded = True
        logger.info(
            f"Connected to {len(self._live_urls)} Audio Flamingo 3 vLLM server(s): "
    ...
    def generate(
        ...
        **kwargs,
        ...
        """
        Send audio to an AF3 vLLM server (round-robin) and parse the tool-call response.
    ...
    def unload_model(self) -> None:
        ...
        self._live_urls = []
        logger.info("Disconnected from Audio Flamingo 3 vLLM server(s)")
