"""
Whisper 3 + Gemma 3 Cascaded Model Adapter

This adapter implements a two-stage pipeline:
1. Whisper 3 (ASR): Audio → Text transcription
2. Gemma 3 (LLM): Text → Tool call

Supports two modes:
- Audio mode: Uses Whisper 3 to transcribe audio, then Gemma 3 for tool calling
- GT mode: Skips Whisper, uses ground truth text directly with Gemma 3

Single server usage:
    # Start Gemma 3 server:
    vllm serve google/gemma-3-12b-it --port 8701 --dtype bfloat16 --trust-remote-code

    # Run benchmark with Whisper transcription:
    python run_benchmark.py --model whisper-gemma3-vllm ...

    # Run benchmark with ground truth text (no Whisper):
    python run_benchmark.py --model whisper-gemma3-vllm-gt ...

Multi-server usage (8 GPUs: 8 servers x 1 GPU each):
    # Server 1 on GPU 0:
    CUDA_VISIBLE_DEVICES=0 vllm serve google/gemma-3-12b-it --port 8701
    # ... up to port 8708

    The adapter auto-discovers servers on ports 8701-8708 by default.
"""
...
class WhisperTranscriber:
    """Whisper 3 transcription component."""
...
class Gemma3VllmClient:
    """Gemma 3 vLLM client component."""
    
    DEFAULT_BASE_PORT = 8701
    DEFAULT_NUM_PORTS = 8
    ...
    def load(self) -> None:
        ...
        if not self._live_urls:
            raise RuntimeError(
                f"No Gemma 3 vLLM servers reachable. Tried: {self._candidate_urls}"
            )
        
        self._is_loaded = True
        logger.info(
            f"Connected to {len(self._live_urls)} Gemma 3 vLLM server(s): "
    ...
    def generate(
        ...
        **kwargs,
        ...
        """Send text to Gemma 3 and get response."""
        payload = {
...
@register_model("whisper-gemma3-vllm")
class WhisperGemma3VllmModel(BaseAudioModel):
    """
    Cascaded Whisper 3 + Gemma 3 model for audio tool calling.
    
    Pipeline: Audio → Whisper 3 (ASR) → Gemma 3 (Tool Call)
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.gemma_model_name = gemma_model
    ...
    def load_model(self) -> None:
        """Load both Whisper and connect to Gemma 3 servers."""
        if self._is_loaded:
            return
        ...
        self._is_loaded = True
        logger.info("Whisper + Gemma 3 pipeline loaded")
    ...
    def generate(
        ...
        **kwargs,
        ...
        2. Send transcription to Gemma 3 for tool calling
        """
        if not self._is_loaded:
        ...
        full_raw = f"[Whisper: {transcription}] [Gemma: {raw_output}]"
    ...
    def unload_model(self) -> None:
        """Unload Whisper and disconnect from Gemma servers."""
        self.whisper.unload()
        self.gemma.unload()
        self._is_loaded = False
        logger.info("Whisper + Gemma 3 pipeline unloaded")


@register_model("whisper-gemma3-vllm-gt")
class WhisperGemma3VllmGTModel(BaseAudioModel):
    """
    Gemma 3 model with Ground Truth text (no Whisper).
    ...
    Pipeline: GT Text → Gemma 3 (Tool Call)
    """
    ...
    def load_model(self) -> None:
        """Connect to Gemma 3 servers."""
        if self._is_loaded:
            return
        
        self.gemma.load()
        self._is_loaded = True
        logger.info("Gemma 3 GT pipeline loaded (no Whisper)")
    ...
    def generate(
        ...
        **kwargs,
        ...
        """
        Process ground truth text through Gemma 3.
        ...
        full_raw = f"[GT: {gt_query}] [Gemma: {raw_output}]"
    ...
    def unload_model(self) -> None:
        """Disconnect from Gemma servers."""
        self.gemma.unload()
        self._is_loaded = False
        logger.info("Gemma 3 GT pipeline unloaded")
